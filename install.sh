...
docker compose exec -T postgres psql -U fusionpbx -d fusionpbx -c "ALTER USER fusionpbx WITH PASSWORD 'password123';" > /dev/null 2>&1

# Install database schema
echo "📊 Installing database schema (this may take 1-2 minutes)..."
docker compose exec -T fusionpbx bash -c 'cd /var/www/fusionpbx && php /var/www/fusionpbx/core/upgrade/upgrade_schema.php' > /dev/null 2>&1

# Create domain
echo "🌐 Creating default domain..."
docker compose exec -T postgres psql -U fusionpbx -d fusionpbx << 'EOF' > /dev/null 2>&1
...
docker compose exec -T fusionpbx bash -c 'cd /var/www/fusionpbx && php /var/www/fusionpbx/core/menu/app_config.php' > /dev/null 2>&1
docker compose exec -T fusionpbx bash -c 'cd /var/www/fusionpbx && php /var/www/fusionpbx/core/upgrade/upgrade_menu.php' > /dev/null 2>&1
docker compose exec -T fusionpbx bash -c 'cd /var/www/fusionpbx && php /var/www/fusionpbx/core/upgrade/upgrade_apps.php' > /dev/null 2>&1
docker compose exec -T fusionpbx bash -c 'cd /var/www/fusionpbx && php /var/www/fusionpbx/core/upgrade/upgrade_domains.php' > /dev/null 2>&1

# Run domain upgrade
echo "📦 Finalizing installation..."
docker compose exec -T fusionpbx bash -c 'cd /var/www/fusionpbx && php /var/www/fusionpbx/core/upgrade/upgrade.php' > /dev/null 2>&1

# Clear cache
echo "🧹 Clearing cache..."
docker compose exec -T fusionpbx bash -c 'cd /var/www/fusionpbx && php /var/www/fusionpbx/core/cache/cache_clear.php' > /dev/null 2>&1
